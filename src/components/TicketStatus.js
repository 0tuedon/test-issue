...
const TicketStatus = () => {
  ...
  const inProgressFiltered =
    tickets.length > 0 &&
    tickets.filter((data) => data.status.includes('In Progress'));
  const resolved =
    tickets.length > 0 &&
    tickets.filter((data) => data.status.includes('Resolved'));
  return (
    <div
      className="flex 
    overflow-x-scroll
    ...
    >
      <DragDropContext onDragEnd={changeStatus}>
        <ProgressSection name={'All'} ticketsData={tickets} />
        <ProgressSection
          name={'In Progress'}
          ticketsData={inProgressFiltered}
        />
        <ProgressSection name={'Resolved'} ticketsData={resolved} />
      </DragDropContext>
    </div>
  );
};
